...
import { addChatWithFB, getMessagesByChatidWithFB } from '../midllewares/middleware';
...
function MessagesScreen() {
	...
	const listItems = messages?.map((message, i) => (
		<ListItem alignItems="flex-start" key={i}>
			<ListItemAvatar>
				<Avatar alt="Remy Sharp" sx={{ bgcolor: '#2196f3' }}>
					{' '}
					{isAuthorBot(message.author) ? <Android /> : <AccountCircle />}{' '}
				</Avatar>
			</ListItemAvatar>
			<ListItemText
				primary={isAuthorBot(message.author) ? 'bot' : message.author}
				secondary={
					<React.Fragment>
				...
				subheader={
					<ListSubheader component="div" id="nested-list-subheader">
						...
						<Dialog open={visible} onClose={handleClose}>
							...
							<TextField placeholder="Chat name" value={chatName} onChange={handleChatName} />
							<Button
								size="small"
								variant="contained"
								endIcon={<Add />}
